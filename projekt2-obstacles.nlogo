...
to catch-sheep
  let prey one-of sheep-here with [gender = "female" and reproduce-energy > 30 and energy > 10]
  let predator one-of sheep-here with [gender = "male" and reproduce-energy > 30 and energy > 10]
...
to catch-wolf
  ...
  let predator one-of wolves-here with [gender = "female" and reproduce-energy > 30 and energy > 10]
  if prey != nobody and predator != nobody
    [
      set energy energy - 20
      set reproduce-energy 5
...
to setup-sheep
  ...
  create-sheep (number-of-sheep / 2)[
    set gender "male"
    set color black
    set energy birth-energy
  ]
...
;;;; procedures added to handle obstacles
; add obstacle by clicking left mouse button
; if turtle is present on clicked patch, turtle dies
to add-obstacles
  while [mouse-down?][
...
; if turtle detects obstacle ahead,
; it turns right for a random amount of degrees in range [90, 270]
to avoid-obstacles
  let target patch-ahead 1
  if [pcolor] of target = red
  [
    right ((random 180) + 90)
  ]
end
